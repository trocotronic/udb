...
ModuleHeader MOD_HEADER(m_message)
  = {
	...
	"$Id: m_message.c,v 1.1.1.1.2.12 2005-02-01 00:30:38 Trocotronic Exp $", /* Version */
...
DLLFUNC int m_message(aClient *cptr, aClient *sptr, int parc, char *parv[], int notice)
{
	    ...
	    nick = strtoken(&p, NULL, ","))
	{
		...
		{
			sendnotice(sptr, "Los comandos IRC no están disponibles porque eres sospechoso de portar un virus");
			continue;
		}
		/*
		   ** nickname addressed?
		...
		if (*nick != '#' && (acptr = find_person(nick, NULL)))
		{
			if (IsVirus(sptr))
			{
				sendnotice(sptr, "Sólo puedes hablar en '%s'", SPAMFILTER_VIRUSCHAN);
				continue;
			}
