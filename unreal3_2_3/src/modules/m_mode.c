...
ModuleHeader MOD_HEADER(m_mode)
  = {
	"m_mode",
	"$Id: m_mode.c,v 1.1.2.6 2005-03-13 21:28:11 Trocotronic Exp $",
	"command /mode", 
	"3.2-b8-1",
	NULL 
    };
...
creationtime = sendts;
	...
	if (IsPerson(sptr) && samode && MyClient(sptr))
	{
		sendto_serv_butone_token(NULL, me.name, MSG_GLOBOPS,
		    TOK_GLOBOPS, ":%s usa SAMODE %s (%s%s%s)", sptr->name,
		    chptr->chname, modebuf, *parabuf ? " " : "", parabuf);
		sendto_failops_whoare_opers
		    ("from %s: %s usa SAMODE %s (%s%s%s)", me.name, sptr->name,
		    chptr->chname, modebuf, *parabuf ? " " : "", parabuf);
		sptr = &me;
		sendts = 0;
	}
	  ...
	  case MODE_NOKNOCK:
		if (what == MODE_ADD && modetype == MODE_NOKNOCK && !(chptr->mode.mode & MODE_INVITEONLY))
		{
			sendto_one(cptr, err_str(ERR_CANNOTCHANGECHANMODE), 
				me.name, cptr->name, 'K', "se requiere +i");
			break;
		}
		goto setthephuckingmode;
	  case MODE_ONLYSECURE:
	  	...
	  	if (what == MODE_ADD && modetype == MODE_ONLYSECURE && !(IsServer(cptr) || IsULine(cptr)))
		{
		  for (member = chptr->members; member; member = member->next)
		  {
			...
			sendto_one(cptr, err_str(ERR_CANNOTCHANGECHANMODE), 
				   me.name, cptr->name, 'z', 
				   "todos los miembros deben estar bajo SSL");
			notsecure = 1;
			break;
		    }
		  }
	  ...
	  case MODE_VOICE:
		      ...
		      && !IsULine(cptr) && !opermode && !samode_in_progress && (what == MODE_DEL))
		  {
			  if (MyClient(cptr))
			  {
				...
				ircsprintf(errbuf, "%s es un fundador de canal", member->cptr->name);
				sendto_one(cptr, err_str(ERR_CANNOTCHANGECHANMODE), me.name, cptr->name,
				   modechar, errbuf);
				break;
			  } else
		      ...
		      && modetype != MODE_CHANOWNER && (what == MODE_DEL))
		  {
			  ...
			  {
				char errbuf[NICKLEN+30];
				ircsprintf(errbuf, "%s es un admin de canal", member->cptr->name);
				sendto_one(cptr, err_str(ERR_CANNOTCHANGECHANMODE), me.name, cptr->name,
				   modechar, errbuf);
				break;
			  } else
					...
					*p2 = '\0';
					...
					for (x = strtok(xbuf+1, ","); x; x = strtok(NULL, ","))
					{
						      ...
						      (*p == 'm') || (*p == 'n') || (*p == 't')))
						{
							if (MyClient(cptr) && *p && (warnings++ < 3))
								sendto_one(cptr, ":%s NOTICE %s :advertencia: modo +f: tipo '%c' desconocido, se ignora.",
									me.name, cptr->name, *p);
						...
						*p = '\0';
						...
						if ((v < 1) || (v > 999)) /* out of range... */
						{
							if (MyClient(cptr))
							{
								sendto_one(cptr, err_str(ERR_CANNOTCHANGECHANMODE),
									   me.name, cptr->name, 
									   'f', "valor debe estar entre 1-999");
								breakit = 1;
								break;
							} else
					...
					if ((v < 1) || (v > 999)) /* 'per' out of range */
					{
						if (MyClient(cptr))
							sendto_one(cptr, err_str(ERR_CANNOTCHANGECHANMODE), 
								   me.name, cptr->name, 'f', 
								   "el tiempo debe estar entre 1-999");
						break;
					}
					newf.per = v;
		...
		switch (*curchr)
		{
		  ...
		  default:
			  ...
			  if (checkrestr && strchr(RESTRICT_CHANNELMODES, *curchr))
			  {
				  if (warnrestr)
				  {
					sendto_one(cptr, ":%s %s %s :El uso de los modos '%s' está deshabilitado.",
...
#ifdef UDB
    	if (MyClient(sptr) && IsShowIp(sptr) && !IsAnOper(sptr)) 
        	sptr->umodes &= ~UMODE_SHOWIP;
    	
        if (MyClient(sptr) && IsServices(sptr) && !IsAnOper(sptr) && !IsHelpOp(sptr))
