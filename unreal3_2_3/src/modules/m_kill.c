...
ModuleHeader MOD_HEADER(m_kill)
  = {
	"kill",	/* Name of module */
	"$Id: m_kill.c,v 1.1.1.1.2.6 2005-02-11 21:38:20 Trocotronic Exp $", /* Version */
...
DLLFUNC int  m_kill(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	    ...
	    nick = strtoken(&p, NULL, ","))
	{
		...
		{
			sendto_one(sptr,
			    ":%s %s %s :*** Demasiados kills. Máximo en %d.",
		...
		if (!IsServer(cptr))
		{
			...
			path = "*sin razón*";	/* Bogus server sending??? */
		/*
