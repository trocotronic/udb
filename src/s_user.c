	...
	if (MyConnect(sptr))
	{
		if ((i = check_client(sptr, username))) {
			...
			sendto_snomask(SNO_CLIENT,
			    ...
			    "conexión no autorizada", get_client_host(sptr));
			ircstp->is_ref++;
			...
			return
			    ...
			    i ==
			    -3 ? mo :
			    "No estás autorizado a conectar a este servidor");
		}
		if (sptr->hostp)
		{
		...
		*ubad = '\0';
		if (strlen(stripuser) != strlen(user->username + noident))
		{
			if (stripuser[0] == '\0')
			{
				return exit_client(cptr, cptr, cptr, "Username incorrecto. Usa sólo 0-9 a-z A-Z _ - . en tu username.");
			}
		    ...
		    CONF_BAN_USER)))
		{
			...
			sendto_one(cptr,
			    ...
			    " Envía un email a %s para más información.",
		...
		if ((bconf = Find_ban(NULL, sptr->info, CONF_BAN_REALNAME)))
		{
			ircstp->is_ref++;
			sendto_one(cptr,
			    ":%s %d %s :*** Tu GECOS (real name) no está permitido en este servidor (%s)."
			    " Cámbiatelo y reconecta",
			    me.name, ERR_YOUREBANNEDCREEP,
			    cptr->name, bconf->reason ? bconf->reason : "");
	...
	else if (IsServer(cptr))
	{
		...
		if (!(acptr = (aClient *)find_server_quick(user->server)))
		{
			...
			return exit_client(sptr, sptr, &me,
			    "USER sin prefijo(2.8) o erróneo");
		}
...
CMD_FUNC(m_nick)
{
	...
	if (acptr)
	{
		 ...
		 */
		sendto_failops("Colisión de nick en %s(%s <- %s)",
		...
		sendto_one(cptr, ":%s KILL %s :%s (%s <- %s)",
		    ...
		    get_client_name(cptr, FALSE));
		sptr->flags |= FLAGS_KILLED;
		return exit_client(cptr, sptr, &me, "Colisión de nick/servidor");
	}
	...
	if (IsServer(sptr))
	{
		...
		sendto_failops("Colisión de nick en %s (%s %ld <- %s %ld)",
		...
		if (!(parc > 3) || (acptr->lastnick == lastnick))
		{
			...
			sendto_serv_butone(NULL,
			    ":%s KILL %s :%s (Colisión de nick)",
			    me.name, acptr->name, me.name);
			acptr->flags |= FLAGS_KILLED;
			(void)exit_client(NULL, acptr, &me,
			    "Colisión de nick por desincronización");
		    ...
		    (!differ && (acptr->lastnick < lastnick)) || acptr->from == cptr)	/* we missed a QUIT somewhere ? */
		{
			...
			sendto_serv_butone(cptr,
			    ...
			    me.name, acptr->name, me.name);
			acptr->flags |= FLAGS_KILLED;
			(void)exit_client(NULL, acptr, &me, "Colisión de nick");
	...
	else
	{
		...
		sendto_failops
		    ("Cambio de nick causa colisión de %s a %s (%s %ld <- %s %ld)",
		...
		if (!(parc > 2) || lastnick == acptr->lastnick)
		{
			...
			sptr->flags |= FLAGS_KILLED;
			acptr->flags |= FLAGS_KILLED;
			(void)exit_client(NULL, sptr, &me, "Colisión propia");
			(void)exit_client(NULL, acptr, &me, "Colisión propia");
		    ...
		    (!differ && (acptr->lastnick < lastnick)))
		{
			...
			ircstp->is_kill++;
			sendto_serv_butone(cptr,
			    ":%s KILL %s :%s (Colisión propia: %s <- %s)",
			    me.name, acptr->name, me.name,
		    ...
		    (!differ && (acptr->lastnick > lastnick)))
		{
			...
			sendto_serv_butone(cptr,
			    ":%s KILL %s :%s (Colisión de nick: %s <- %s)",
			    me.name, sptr->name, me.name,
			    sptr->from->name, acptr->from->name);
			sptr->flags |= FLAGS_KILLED;
			(void)exit_client(NULL, sptr, &me, "Colisión de nick");
			/*
			 * Introduce our "correct" user to the other server
			 */
			/* Kill their user. */
	...
	{		
		...
		sendto_one(sptr,
		    ...
		    " envía un email a " CONTACT_EMAIL
		    " con la versión de tu cliente"
		  ...
		  case 'x':
			  switch (UHOST_ALLOWED)
			  {
				...
				case UHALLOW_NOCHANS:
					...
					{
						if (!modex_err) {
							sendto_one(sptr, ":%s %s %s :*** No puedes usar %cx mientras estés en canales", me.name, IsWebTV(sptr) ? "PRIVMSG" : "NOTICE", sptr->name, what == MODE_ADD ? '+' : '-');
							modex_err = 1;
						}
						break;
					}
