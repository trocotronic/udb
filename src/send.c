...
int  send_queued(aClient *to)
{
	...
	*/
	...
	while (DBufLength(&to->sendQ) > 0)
	{
		...
		{
			char buf[256];
			snprintf(buf, 256, "Error de escritura: %s", STRERROR(ERRNO));
...
void sendbufto_one(aClient *to, char *msg, unsigned int quick)
{
	...
	{
		if (IsServer(to))
			sendto_ops("Límite de Max SendQ excedido por %s: %u > %d",
			    get_client_name(to, FALSE), DBufLength(&to->sendQ),
			    get_sendq(to));
		dead_link(to, "Max SendQ excedido");
		return;
	}
...
void sendto_connectnotice(char *nick, anUser *user, aClient *sptr, int disconnect, char *comment)
{
	...
	if (!disconnect)
	{
		...
		ircsprintf(connectd,
		    "*** Notice -- Cliente conecta en puerto %d: %s (%s@%s) [%s] %s%s%s",
...
#endif
		ircsprintf(connecth,
		    "*** Notice -- Cliente conecta: %s (%s@%s) [%s] {%s}", nick,
		    user->username, user->realhost, Inet_ia2p(&sptr->ip),
		    sptr->class ? sptr->class->name : "0");
	}
	else 
	{
		ircsprintf(connectd, "*** Notice -- Cliente cierra: %s (%s@%s) [%s]",
			nick, user->username, user->realhost, comment);
		ircsprintf(connecth, "*** Notice -- Cliente cierra: %s (%s@%s) [%s] [%s]",
...
void sendto_fconnectnotice(char *nick, anUser *user, aClient *sptr, int disconnect, char *comment)
{
	...
	if (!disconnect)
	{
		ircsprintf(connectd, "*** Notice -- Cliente conecta en %s: %s (%s@%s)",
			    user->server, nick, user->username, user->realhost);
		ircsprintf(connecth,
		    "*** Notice -- Cliente conecta en %s: %s (%s@%s) [%s] {0}", user->server, nick,
		    user->username, user->realhost, user->ip_str ? user->ip_str : "0");
	}
	else 
	{
		ircsprintf(connectd, "*** Notice -- Cliente cierra en %s: %s!%s@%s (%s)",
			   user->server, nick, user->username, user->realhost, comment);
		ircsprintf(connecth, "*** Notice -- Cliente cierra en %s: %s (%s@%s) [%s] [%s]",
			user->server, nick, user->username, user->realhost, comment,
