	...
	switch (*parv[1])
	{
	  case '+':
	  {
		  ...
		  if (type & TKL_SPAMF)
		  {
		  	  char buf[512];
			  snprintf(buf, 512,
			      "Spamfilter añadido: '%s' [target: %s] [acción: %s] el %s GMT (de %s)",
			  ...
			  {
				   sendto_snomask(SNO_TKL,
			     		 "*** %s añadida a %s@%s (desde %s GMT a %s: %s)",
			  ...
			  {
				  sendto_snomask(SNO_TKL,
			      		"*** %s permanente añadida a %s@%s (desde %s GMT a %s: %s)",
			      		txt, parv[3], parv[4], gmt, gmt2,
			    		  parv[8]);
				  ircd_log
	...
	for (tk = tklines[tkl_hash('F')]; tk; tk = tk->next)
	{
		...
		if (!regexec(&tk->spamf->expr, str, 0, NULL, 0))
		{
			...
			if (target) {
				...
				targetbuf[0] = '\0';
			ircsprintf(buf, "[Spamfilter] %s!%s@%s concuerda con '%s': [%s%s: '%s']",
