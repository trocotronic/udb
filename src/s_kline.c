...
aTKline *tkl_expire(aTKline * tmp)
{
	...
	if (!(tmp->type & TKL_NICK))
	{
		sendto_snomask(SNO_TKL,
		    "*** Expira %s (%s@%s) puesta por %s (Razón: %s) hace %li segundos",
	...
	{
		sendto_snomask(SNO_TKL,
			"*** Expira %s (%s) puesta por %s (Razón: %s) hace %li segundos",
...
int  find_tkline_match(aClient *cptr, int xx)
{
	...
	if ((lp->type & TKL_KILL) && (xx != 2))
	{
		if (lp->type & TKL_GLOBAL)
		{
			ircstp->is_ref++;
			if (GLINE_ADDRESS)
				sendto_one(cptr, ":%s NOTICE %s :*** Estas %s de %s (%s)"
					   " Email %s para más información.",
					   me.name, cptr->name,
...
aTKline *find_qline(aClient *cptr, char *nick, int *ishold)
{
	...
	*ishold = 0;
	...
	chost = cptr->user ? cptr->user->realhost : (MyConnect(cptr) ? cptr->sockhost : "desconocido");
	cname = cptr->user ? cptr->user->username : "desconocido";
	strcpy(host, make_user_host(cname, chost));
		  ...
		  if (type & TKL_SPAMF)
		  {
			...
			char buf[512];
			  if (expiry_1 != 0)
			  {
				...
				{
					if (*parv[3] != 'H')
						snprintf(buf, 512, "%s añadida a %s (desde %s GMT a %s: %s)",
							txt, parv[4], gmt, gmt2, reason);
				}
				else
					snprintf(buf, 512, "%s añadida a %s@%s (desde %s GMT a %s: %s)",
						txt, parv[3], parv[4], gmt, gmt2, reason);
			  }
			  else
			  {
				if (type & TKL_NICK)
				{
					if (*parv[3] != 'H')
						snprintf(buf, 512, "%s permanente añadida a %s (desde %s GMT a %s: %s)",
							txt, parv[4], gmt, gmt2, reason);
				}
				else
					snprintf(buf, 512, "%s permanente añadida a %s@%s (desde %s GMT a %s: %s)",
						txt, parv[3], parv[4], gmt, gmt2, reason);
			  }
				...
				if (!stricmp(tk->hostmask, parv[4]) && !stricmp(tk->usermask, parv[3]))
				  ...
				  if (match)
				  {
					  ...
					  if (type & TKL_NICK)
					  {
						if (!(*parv[3] == 'H'))
						{
							sendto_snomask(SNO_TKL, "%s quitada %s %s (hace %s - razón: %s)",
					  ...
					  {
					  	  sendto_snomask(SNO_TKL, "%s quita Spamfilter '%s' (puesto %s)",
					  	     parv[5], tk->reason, gmt);
					  	  ircd_log(LOG_TKL, "%s removed Spamfilter '%s' (set at %s)",
					  	     parv[5], tk->reason, gmt);
					  } else {
						  sendto_snomask(SNO_TKL,
						      "%s quita %s %s@%s (puesto %s - razón: %s)",
...
int place_host_ban(aClient *sptr, int action, char *reason, long duration)
{
	switch(action)
	{
		case BAN_ACT_TEMPSHUN:
			...
			sendto_snomask(SNO_TKL, "Shun temporal añadido a %s (%s@%s) [%s]",
				sptr->name,
...
char *str;
	...
	for (tk = tklines[tkl_hash('F')]; tk; tk = tk->next)
	{
		...
		if (!regexec(&tk->ptr.spamf->expr, str, 0, NULL, 0))
		{
			...
			ircsprintf(buf, "[Spamfilter] %s!%s@%s concuerda con '%s': [%s%s: '%s'] [%s]",
			...
			if (tk->ptr.spamf->action == BAN_ACT_BLOCK)
			{
				switch(type)
				{
					...
					case SPAMF_USERNOTICE:
						sendnotice(sptr, "Mensaje a %s bloqueado: %s",
							target, unreal_decodespace(tk->ptr.spamf->tkl_reason));
						break;
					case SPAMF_CHANMSG:
					case SPAMF_CHANNOTICE:
						sendto_one(sptr, ":%s 404 %s %s :Mensaje bloqueado: %s",
							me.name, sptr->name, target,
							unreal_decodespace(tk->ptr.spamf->tkl_reason));
						break;
					case SPAMF_DCC:
						sendnotice(sptr, "DCC a %s bloqueado: %s",
							target, unreal_decodespace(tk->ptr.spamf->tkl_reason));
						break;
					case SPAMF_AWAY:
					...
					case SPAMF_TOPIC:
						//...
						sendnotice(sptr, "El topic para %s con este texto está bloqueado: %s",
							target, unreal_decodespace(tk->ptr.spamf->tkl_reason));
						break;
					default:
						break;
			...
			if (tk->ptr.spamf->action == BAN_ACT_DCCBLOCK)
			{
				if (type == SPAMF_DCC)
				{
					sendnotice(sptr, "DCC a %s bloqueado: %s",
						target, unreal_decodespace(tk->ptr.spamf->tkl_reason));
					sendnotice(sptr, "*** Has sido bloqueado para enviar archivos DCC, "
					           "reconecta para volver a poder enviar archivos");
			...
			if (tk->ptr.spamf->action == BAN_ACT_VIRUSCHAN)
			{
				...
				sendnotice(sptr, "No puedes hablar en %s: %s",
				...
				if (chptr)
				{
					...
					ircsprintf(buf, "[Spamfilter] %s concuerda con '%s' [%s%s] [%s]",
