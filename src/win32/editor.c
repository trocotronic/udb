...
LRESULT CALLBACK FromFileDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) 
{
		...
		if (fr->Flags & FR_FINDNEXT)
		{
			...
			if (SendMessage(hRich, EM_FINDTEXTEX, flags, (LPARAM)&ft) == -1)
				MessageBox(NULL, "Unreal ha terminado de buscar en el documento",
					"Buscar", MB_ICONINFORMATION|MB_OK);
			else
			{
	...
	switch (message) 
	{
		case WM_INITDIALOG: 
		{
			...
			if (file)
				wsprintf(szText, "Editor UnrealIRCd - %s", file);
			else 
			...
			case TTN_GETDISPINFO: 
			{
				...
				switch (lpttt->hdr.idFrom) 
				{
					...
					case IDC_BOLD:
						...
						break;
					case IDC_UNDERLINE:
						strcpy(lpttt->szText, "Subrayado");
						break;
					case IDC_COLOR:
						strcpy(lpttt->szText, "Texto Color");
						break;
					case IDC_BGCOLOR:
						strcpy(lpttt->szText, "Fondo Color");
						break;
					case IDC_GOTO:
		...
		case WM_COMMAND:
			...
			if (LOWORD(wParam) == IDM_NEW) 
			{
				...
				{
					sprintf(text, "El texto en el archivo %s ha cambiado.\r\n\r\n¿Quiere guardar los cambios?", file ? file : "nuevo");
