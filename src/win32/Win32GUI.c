...
static HMENU hRehash, hAbout, hConfig, hTray, hLogs;
	...
	switch (message)
			...
			case WM_CLOSE: {
				if (MessageBox(hDlg, "¿Quieres cerrar UnrealIRCd?", "¿Estás seguro?", MB_YESNO|MB_ICONQUESTION) == IDNO)
					return 0;
				else {
					DestroyWindow(hDlg);
					exit(0);
...
(DLGPROC)FromFileReadDLG, (LPARAM)path);
				...
				switch(LOWORD(wParam)) {
					...
					case IDM_RHCONF:
						...
						sendto_realops("Refrescando archivo de configuración vía consola");
						rehash(&me,&me,0);
						break;
					case IDM_RHMOTD: {
						ConfigItem_tld *tlds;
						...
						rehash_motdrules();
						sendto_realops("Refrescando todos los MOTD y archivos RULES vía consola");
						break;
					}
					case IDM_RHOMOTD:
						MessageBox(NULL, "Refrescando OPERMOTD", "Refrescando", MB_OK);
						opermotd = (aMotd *) read_file(OPATH, &opermotd);
						sendto_realops("Refrescando OPERMOTD vía consola");
						break;
					case IDM_RHBMOTD:
					...
					case IDM_DAL:
						...
						break;
					case IDM_HELP:
						DialogBox(hInst, "Ayuda", hDlg, (DLGPROC)HelpDLG);
						break;
					case IDM_CONF:
						DialogBoxParam(hInst, "FromFile", hDlg, (DLGPROC)FromFileDLG, 
							(LPARAM)CPATH);
...
LRESULT CALLBACK CreditsDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	return FromVarDLG(hDlg, message, wParam, lParam, "Créditos UnrealIRCd", unrealcredits);
}

LRESULT CALLBACK DalDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	return FromVarDLG(hDlg, message, wParam, lParam, "DALnet Créditos UnrealIRCd", dalinfotext);
}
...
LRESULT CALLBACK FromFileDLG(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (message == WM_FINDMSGSTRING)
	{
		...
		if (fr->Flags & FR_FINDNEXT)
		{
			...
			{
				MessageBox(NULL, "Unreal ha terminado de buscar en el documento",
	...
	switch (message) {
		case WM_INITDIALOG: {
			...
			if (file)
				...
				strcpy(szText, "UnrealIRCd Editor - Nuevo");
			SetWindowText(hDlg, szText);
			lpfnOldWndProc = (FARPROC)SetWindowLong(hWnd, GWL_WNDPROC, (DWORD)RESubClassFunc);
		...
		case WM_NOTIFY:
			switch (((NMHDR *)lParam)->code) {
				...
				if (currline != prevline) {
					wsprintf(buffer, "Línea: %d", currline);
			...
			case TTN_GETDISPINFO: {
				...
				switch (lpttt->hdr.idFrom) {
					case IDM_NEW:
						strcpy(lpttt->szText, "Nuevo");
						break;
					case IDM_SAVE:
						strcpy(lpttt->szText, "Guardar");
						break;
					case IDM_CUT:
						strcpy(lpttt->szText, "Cortar");
						break;
					case IDM_COPY:
						strcpy(lpttt->szText, "Copiar");
						break;
					case IDM_PASTE:
						strcpy(lpttt->szText, "Pegar");
						break;
					case IDM_UNDO:
						strcpy(lpttt->szText, "Deshacer");
						break;
					case IDM_REDO:
						strcpy(lpttt->szText, "Rehacer");
						break;
					case IDC_BOLD:
						strcpy(lpttt->szText, "Negrita");
						break;
					case IDC_UNDERLINE:
						strcpy(lpttt->szText, "Subrayar");
						break;
					case IDC_COLOR:
					...
					case IDC_BGCOLOR:
						...
						break;
					case IDC_GOTO:
						strcpy(lpttt->szText, "Ir a");
						break;
					case IDC_FIND:
						strcpy(lpttt->szText, "Buscar");
						break;
				}
				return (TRUE);
			}
			case NM_DBLCLK:
				DialogBox(hInst, "Ir a", hDlg, (DLGPROC)GotoDLG);
				return (TRUE);
			}
		...
		if (LOWORD(wParam) == IDM_NEW) {
			...
			if (SendMessage(GetDlgItem(hDlg, IDC_TEXT), EM_GETMODIFY, 0, 0) != 0) {
				sprintf(text, "El archivo %s ha cambiado.\r\n\r\n¿Quieres guardar los cambios?", file ? file : "nuevo");
