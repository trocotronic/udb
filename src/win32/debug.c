...
LONG __stdcall ExceptionFilter(EXCEPTION_POINTERS *e) {
	...
	sprintf(text, "UnrealIRCd ha encontrado un error. La información ha sido escrita en"
		" wircd.%d.core, envía un email con este archivo a los desarrolladores.",
		getpid());
	fclose(fd);
	if (!IsService)
		...
		" wircd.%d.core, envía un email con este archivo a los desarrolladores.", getpid());
		fclose(fd);
	}
	CleanUp();
