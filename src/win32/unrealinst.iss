...
[Setup]
AppName=UnrealIRCd
AppVerName=UnrealIRCd3.2.8-rc1+UDB 3.6.1
    ...
    if (NOT FileExists(msvcrt)) then begin
      ...
      if answer = IDYES then begin
        ...
        if isxdl_Download(hWnd, crturl, tmp) = 0 then begin
          MsgBox('La descarga e instalación de msvcr71.dll han fallado. El archivo tiene que instalarse manualmente. Puede descargarlo de http://www.unrealircd.com/downloads/mscvr71.dll', mbInformation, MB_OK);
        end else
          didCrtDl := true;
      end else
        MsgBox('Esta librería es necesaria. Puede descargarla de http://www.unrealircd.com/downloads/msvcr71.dll', mbInformation, MB_OK);
    end;
    if (NOT FileExists(msvcpt)) then begin
      ...
      if answer = IDYES then begin
        ...
        if isxdl_Download(hWnd, cpturl, tmp) = 0 then begin
          MsgBox('La descarga e instalación de msvcp71.dll han fallado. El archivo tiene que instalarse manualmente. Puede descargarlo de http://www.unrealircd.com/downloads/mscvp71.dll', mbInformation, MB_OK);
        end else
          didCrtDl := true;
      end else
        MsgBox('Esta librería es necesaria. Puede descargarla de http://www.unrealircd.com/downloads/msvcp71.dll', mbInformation, MB_OK);
    end;
