...
void server_reboot(char *mesg)
{
	...
	aClient *cptr;
#endif
	sendto_realops("Reiniciando servidor... %s", mesg);
	Debug((DEBUG_NOTICE, "Reiniciando servidor... %s", mesg));
#ifndef _WIN32
	flush_connections(&me);
#else
	for (i = LastSlot; i >= 0; i--)
...
#endif
#ifndef _WIN32
	Debug((DEBUG_FATAL, "No se pudo reiniciar el servidor: %s", strerror(errno)));
#else
	Debug((DEBUG_FATAL, "No se pudo reiniciar el servidor: %s",
...
EVENT(garbage_collect)
{
	...
	if (loop.do_garbage_collect == 1)
		sendto_realops("Haciendo colección garbage ..");
	if (freelinks > HOW_MANY_FREELINKS_ALLOWED) {
		...
		if (loop.do_garbage_collect == 1) {
			...
			sendto_realops
			    ("Limpiados %i bloques garbage", (ii - freelinks));
		}
	}
	if (loop.do_garbage_collect == 1)
		loop.do_garbage_collect = 0;
...
static TS try_connections(TS currenttime)
{
	...
	connecting = FALSE;
	Debug((DEBUG_NOTICE, "Conexión comprobada en   : %s",
	    myctime(currenttime)));
	...
	Debug((DEBUG_NOTICE, "Próxima comprobación de conexión: %s", myctime(next)));
	return (next);
}
...
extern TS check_pings(TS currenttime)
{
	...
	for (i = 0; i <= LastSlot; i++) {
		...
		if (loop.do_bancheck) {
			...
			if (killflag) {
				if (IsPerson(cptr))
					sendto_realops("Ban activo para %s (%s)",
		...
		if (ping <= (currenttime - cptr->lasttime))
		{
				...
				)
			{
				...
				if (IsServer(cptr) || IsConnecting(cptr) ||
					...
					sendto_realops
					    ("Sin respuesta de %s. Se cierra link.",
					    get_client_name(cptr, FALSE));
					sendto_serv_butone(&me,
					    ":%s GLOBOPS :Sin respuesta de %s. Se cierra link.",
...
#endif
				exit_client(cptr, cptr, &me, "Ping fuera de tiempo");
				continue;
...
#define CMDLINE_CFG ""
	...
	if (!IsService) {
		MessageBox(NULL,
		    "Uso: wircd [-h servidor] [-p puerto] [-x nivel_log]\n",
		    "UnrealIRCD/32", MB_OK);
	}
#endif
	return (-1);
...
# endif
	...
	ircd_res_init();
	if (chroot(DPATH)) {
		(void)fprintf(stderr, "ERROR:  No puede hacer chdir(chroot) a '%s'\n", dpath);
		exit(5);
	}
#endif	 /*CHROOTDIR*/
	    myargv = argv;
		...
		switch (flag) {
		  ...
		  case 'h':
			  if (!strchr(p, '.')) {
				  ...
				  (void)printf
				      ("ERROR: %s inválido. El servidor tiene que contener almenos un \".\"\n",
				      p);
				  exit(1);
			  }
		  ...
		  case 'P':{
			  ...
			  if ((type = Auth_FindType(p)) == -1) {
				  printf("No se encuentra método de autentificación %s\n", p);
				  exit(0);
			  }
			  p = *++argv;
			  argc--;
			  ...
			  printf("El password encriptado es: %s\n", result);
			  exit(0);
		  }
#endif
		  ...
		  case 'v':
			  if (!IsService) {
				  MessageBox(NULL, version,
				      "Versión UnrealIRCD/Win32", MB_OK);
			  }
#endif
			  exit(0);
		  case 'W':{
...
# ifndef _WIN32
			  (void)fprintf(stderr,
			      "%s: DEBUGMODE definido por -x y\n",
			      myargv[0]);
# else
			  if (!IsService) {
				  MessageBox(NULL,
				      "DEBUGMODE definido por -x",
				      "UnrealIRCD/32", MB_OK);
			  }
# endif
			  exit(0);
...
/* Hack to stop people from being able to read the config file */
	...
	{
		config_error("Faltan módulos con comandos correctos. LEE LA DOCUMENTACIÓN.");
		exit(-4);
	}

#ifdef USE_SSL
#ifndef _WIN32
	fprintf(stderr, "* Iniciando SSL.\n");
#endif
	init_ssl();
#endif
#ifndef _WIN32
	fprintf(stderr,
	    "* Configuración dinámica cargada ... iniciando IRCd.\n");
	fprintf(stderr,
	    "---------------------------------------------------------------------\n");
#endif
	open_debugfile();
...
#endif
	init_random();
	Debug((DEBUG_NOTICE, "Servidor preparado..."));
	SetupEvents();
#ifdef THROTTLING
