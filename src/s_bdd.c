...
void EnviaADebugs(aClient *sptr, int comando, char *param)
{
	...
	{
		if ((acptr = local[i]) && IsServer(acptr) && MyConnect(acptr) && (acptr != sptr) && *param) 
		{
...
aClient *ChanClient()
{
	...
	if ((chanserv = BuscaBloque(S_CHA, UDB_SET)) && !BadPtr(chanserv->data_char))
