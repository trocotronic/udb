...
add_con_refuse:
		...
		for (i = LastSlot; i >= 0; i--)
		{
...
#endif
			{
				...
				{
					ircsprintf(zlinebuf,
						"ERROR :Cierra link: [%s] (Demasiadas conexiones desconocidas desde tu IP)"
						"\r\n",
						Inet_ia2p(&acptr->ip));
		...
		if ((bconf = Find_ban(acptr->sockhost, CONF_BAN_IP))) {
			...
			{
				ircsprintf(zlinebuf,
					"ERROR :Cierra Link: [%s] (No eres bienvenido en este servidor: %s\r\n ", 
					Inet_ia2p(&acptr->ip),
					bconf->reason ? bconf->reason : "sin razón");
				set_non_blocking(fd, acptr);
				set_sock_opts(fd, acptr);
				send(fd, zlinebuf, strlen(zlinebuf), 0);
				goto add_con_refuse;
...
static int read_packet(aClient *cptr, fd_set *rfd)
{
	...
	else
	{
		...
		)
		{
			...
			return exit_client(cptr, cptr, cptr, "Exceso de flood");
		}
...
deadsocket:
		...
		if (length != -2 && (IsServer(cptr) || IsHandshake(cptr)))
		{
			if (length == 0)
			{
				...
				report_baderror("Conexión perdida con %s:%s", cptr);
		}
...
/* FIXME: no ZIP link handling here, but this code doesnt work anyway -- Syzop */
	    ...
	    i++, pfd++)
	{
		...
		if (rr && IsListening(cptr))
		{
			...
			{
				report_error("No acepta conexiones %s:%s",
				    cptr);
				break;
			}
			ircstp->is_ac++;
		...
		if (length != -2 && (IsServer(cptr) || IsHandshake(cptr)))
		{
			if (length == 0)
			{
				sendto_locfailops
				    ...
				    get_client_name(cptr, FALSE));
				sendto_serv_butone(&me,
				    ":%s GLOBOPS :Servidor %s cierra la conexiónn",
				    me.name, get_client_name(cptr, FALSE));
			}
			else
				report_error("Conexión perdida con %s:%s", cptr);
		}
		if (length != FLUSH_BUFFER)
			(void)exit_client(cptr, cptr, &me,
			    ((sockerr = get_sockerr(cptr))
...
#endif
		errtmp = ERRNO;
		report_error("Conexión a %s fallida: %s", cptr);
		if (by && IsPerson(by) && !MyClient(by))
			sendto_one(by,
			    ":%s NOTICE %s :*** Conexión a %s fallida.",
