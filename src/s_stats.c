...
int stats_tablas(aClient *sptr, char *para)
{
	...
	for (bdd = PRIMERA_LETRA; bdd <= ULTIMA_LETRA; bdd++) 
	{
		...
		{
			if (IsOper(sptr))
				sendto_one(sptr, ":%s %i %s b Tabla '%c' :S=%09lu R=%09lu H=%09lu %s", me.name, RPL_STATSDEBUG, sptr->name, bdd, registros[SERS][bdd], registros[REGS][bdd], registros[HASH][bdd], registros[CORR][bdd] ? "(Corrupta)" : "");
			else
				sendto_one(sptr, ":%s %i %s b Tabla '%c' :S=%09lu R=%09lu", me.name, RPL_STATSDEBUG, sptr->name, bdd, registros[SERS][bdd], registros[REGS][bdd]);
		}
		else
		{
			if (registros[SERS][bdd])
				sendto_one(sptr, ":%s %i %s b Tabla '%c' :S=%09lu NoResidente", me.name, RPL_STATSDEBUG, sptr->name, bdd, registros[SERS][bdd]);
		}
	}
	return 0;
}
