...
ModuleHeader MOD_HEADER(m_tkl)
  = {
	"tkl",	/* Name of module */
	"$Id: m_tkl.c,v 1.1.1.9 2006-02-15 22:06:20 Trocotronic Exp $", /* Version */
...
int remove = 0;
	...
	if (!MyClient(acptr))
	{
		...
		if (!remove)
		{
			if (IsShunned(acptr))
			{
				sendnotice(sptr, "El usuario '%s' ya está bajo shun", acptr->name);
			} else if (IsAnOper(acptr))
			{
				sendnotice(sptr, "No puedes poner bajo shun a '%s' porque es un Operador", acptr->name);
			} else
			{
				SetShunned(acptr);
				ircsprintf(buf, "Shun temporal añadido para %s (%s@%s) por %s [%s]",
			...
			if (!IsShunned(acptr))
			{
				sendnotice(sptr, "El usuario '%s' no está bajo shun", acptr->name);
			} else {
				ClearShunned(acptr);
				ircsprintf(buf, "Se quita el shun de %s (%s@%s) por %s",
					acptr->name, acptr->user->username, acptr->user->realhost,
					sptr->name);
				sendto_snomask(SNO_TKL, "%s", buf);
				sendto_serv_butone_token(NULL, me.name, MSG_SENDSNO, TOK_SENDSNO, "G :%s", buf);
...
DLLFUNC int  m_tkl_line(aClient *cptr, aClient *sptr, int parc, char *parv[], char* type) {
	...
	if (strchr(mask, '!'))
	{
		sendto_one(sptr, ":%s NOTICE %s :[error] Las máscaras no pueden contener el caracter '!'.", me.name,
		    sptr->name);
		return 0;
	}
	if (*mask == ':')
	{
		sendto_one(sptr, ":%s NOTICE %s :[error] La máscara no puede empezar por ':'.", me.name,
			sptr->name);
		return 0;
	}
	if (strchr(mask, ' '))
	...
	if (p) {
		if ((p == mask) || !p[1])
		{
			sendnotice(sptr, "Error: especifique user@host");
			return 0;
		}
		...
		if (*hostmask == ':')
		{
			sendnotice(sptr, "[error] Por razones internas, el host no puede empezar por ':'");
			return 0;
		}
		if (((*type == 'z') || (*type == 'Z')) && !whattodo)
		{
			...
			if (strcmp(usermask, "*"))
			{
				sendnotice(sptr, "ERROR: (g)zlines deben ponerse a \037*\037@ipmask, no \037user\037@ipmask. Esto es "
				                 "porque (g)zlines se procesan ANTES de la resolución dns e ident.. "
				                 "Si quiere usar máscaras de usuario, use KLINE/GLINE.");
				return -1;
			}
			for (p=hostmask; *p; p++)
				if (isalpha(*p))
				{
					sendnotice(sptr, "ERROR: (g)zlines deben ponerse a *@\037IPMASK\037, no *@\037HOSTMASK\037 "
					                 ...
					                 "Esto es porque (g)zlines se procesan ANTES de la resolución dns e ident.. "
					                 "Si quiere usar hosts en vez de ips, use KLINE/GLINE.");
					return -1;
				}
		}
	...
	else
	{
		...
		if ((acptr = find_person(mask, NULL)))
		{
			...
			if ((*type == 'z') || (*type == 'Z'))
			{
				...
				if (!hostmask)
				{
					sendnotice(sptr, "No se puede obtener la IP de '%s'", acptr->name);
					return 0;
				}
			} else {
				/* Fill in host */
	...
	if (whattodo == 0 && (parc > 3))
	{
		...
		{
			sendto_one(sptr,
			    ":%s NOTICE %s :*** [error] Tiempo inválido",
			    me.name, sptr->name);
			return 0;
		}
	}
	...
	if (whattodo == 0)
	{
		...
		{
			sendto_one(sptr,
				":%s NOTICE %s :*** [error] Tiempo fuera de rango",
				me.name, sptr->name);
			return 0;
		}
...
{
	sendnotice(sptr, "Uso: /spamfilter [add|del|remove|+|-] [tipo] [acción] [tiempo] [razón] [regex]");
	sendnotice(sptr, "Vea '/helpop ?spamfilter' para más información.");
	return 0;
}
...
int n;
	...
	{
		sendto_one(sptr, ":%s NOTICE %s :El primer parámetro es incorrecto", me.name, sptr->name);
		return spamfilter_usage(sptr);
	}
	...
	if (!action)
	{
		sendto_one(sptr, ":%s NOTICE %s :Acción '%s' incorrecta", me.name, sptr->name, parv[3]);
		return spamfilter_usage(sptr);
	}
	...
	if (p)
	{
		sendto_one(sptr, ":%s NOTICE %s :Error en la expresión '%s': %s",
...
aTKline *_tkl_expire(aTKline * tmp)
{
	...
	if (!(tmp->type & TKL_NICK))
	{
		sendto_snomask(SNO_TKL,
		    "*** Expira %s (%s@%s) puesta por %s (Razón: %s) hace %li segs",
	...
	{
		sendto_snomask(SNO_TKL,
			"*** Expira %s (%s) puesta por %s (Razón: %s) hace %li segs",
...
int  _find_tkline_match(aClient *cptr, int xx)
{
	...
	if ((lp->type & TKL_KILL) && (xx != 2))
	{
		if (lp->type & TKL_GLOBAL)
		{
			ircstp->is_ref++;
			if (GLINE_ADDRESS)
				sendto_one(cptr, ":%s NOTICE %s :*** Estás %s de %s (%s)"
					   " Póngase en contacto con %s para más información.",
					   me.name, cptr->name,
		...
		else
		{
			...
			ircsprintf(msge, "Glined %s (%s)", lp->reason);
			return (exit_client(cptr, cptr, &me, msge));
...
aClient *acptr;
	...
	for (i = LastSlot; i >= 0; i--)
		if ((acptr = local[i]) && MyClient(acptr))
		{
			...
			/* matched! */
			ircsprintf(buf, "[Spamfilter] %s!%s@%s coincide con el filtro '%s': [%s: '%s'] [%s]",
				acptr->name, acptr->user->username, acptr->user->realhost,
				tk->reason,
...
aClient *acptr;
	...
	for (acptr = client; acptr; acptr = acptr->next)
		if (IsPerson(acptr))
		{
			...
			/* matched! */
			sendnotice(from, "[Spamfilter] %s!%s@%s coincide con el filtro '%s': [%s: '%s'] [%s]",
				acptr->name, acptr->user->username, acptr->user->realhost,
				tk->reason,
				"user", spamfilter_user,
				unreal_decodespace(tk->ptr.spamf->tkl_reason));
	...
	switch (*parv[1])
	{
	  case '+':
	  {
		  ...
		  if (type & TKL_SPAMF)
		  {
		  	  char buf[512];
			  snprintf(buf, 512,
			      "Spamfilter añadido: '%s' [target: %s] [acción: %s] [razón: %s] el %s GMT (desde %s)",
			...
			char buf[512];
			  if (expiry_1 != 0)
			  {
				...
				{
					if (*parv[3] != 'H')
						snprintf(buf, 512, "%s añadia a %s por %s (desde %s hasta %s GMT: %s)",
							txt, parv[4], parv[5], gmt2, reason);
				}
				else
					snprintf(buf, 512, "%s añadida a %s@%s por %s (desde %s hasta %s GMT: %s)",
						txt, parv[3], parv[4], parv[5], gmt, gmt2, reason);
			  }
			  else
			  {
				if (type & TKL_NICK)
				{
					if (*parv[3] != 'H')
						snprintf(buf, 512, "%s permanente añadida a %s por %s (desde %s GMT: %s)",
							txt, parv[4], parv[5], gmt, reason);
				}
				else
					snprintf(buf, 512, "%s permanente añadida a %s@%s por %s (desde %s GMT: %s)",
						txt, parv[3], parv[4], parv[5], gmt, reason);
			  }
				...
				if (!stricmp(tk->hostmask, parv[4]) && !stricmp(tk->usermask, parv[3]))
				  ...
				  if (match)
				  {
					  ...
					  if (type & TKL_NICK)
					  {
						if (!(*parv[3] == 'H'))
						{
							sendto_snomask(SNO_TKL, "%s quita %s a %s (puesta en %s GMT: %s)",
								parv[5], txt, tk->hostmask, gmt, tk->reason);
							ircd_log(LOG_TKL, "%s quita %s a %s (puesta en %s GMT: %s)",
								parv[5], txt, tk->hostmask, gmt, tk->reason);
						}
					  }
					  else if (type & TKL_SPAMF)
					  {
					  	  sendto_snomask(SNO_TKL, "%s quita Spamfilter (puesta en %s GMT: %s)",
					  	     parv[5], gmt, tk->reason);
					  	  ircd_log(LOG_TKL, "%s removed Spamfilter '%s' (set at %s)",
					  	     parv[5], tk->reason, gmt);
					  } else {
						  sendto_snomask(SNO_TKL,
						      "%s quita %s a %s@%s (puesta en %s GMT: %s)",
...
char *str;
	 ...
	 */
#ifdef UDB
	if (!sptr->user || IsHOper(sptr) || IsULine(sptr))
#else
