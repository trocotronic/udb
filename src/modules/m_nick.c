...
ModuleHeader MOD_HEADER(m_nick)
  = {
	"m_nick",
	"$Id: m_nick.c,v 1.1.4.1 2005-03-21 10:36:55 Trocotronic Exp $",
	"command /nick", 
	"3.2-b8-1",
	NULL 
    };
...
DLLFUNC CMD_FUNC(m_nick)
{
	...
	if (!IsServer(sptr) && strchr(parv[1],'!') && acptr != sptr && val > 1) 
	{
		...
		sendto_serv_butone_token(NULL, me.name, MSG_KILL, TOK_KILL, "%s :Sesión fantasma liberada por %s", acptr->name, who);
		if (MyClient(acptr))
			sendto_one(acptr, ":%s KILL %s :Sesión fantasma liberada por %s", me.name, acptr->name, who);
		acptr->flags |= FLAGS_KILLED;
