...
ModuleHeader MOD_HEADER(m_connect)
  = {
	"m_connect",
	"$Id: m_connect.c,v 1.1.4.1 2004-03-08 18:07:06 Trocotronic Exp $",
	"command /connect", 
	"3.2-b8-1",
	NULL 
    };
...
DLLFUNC CMD_FUNC(m_connect)
{
	...
	{
		sendto_one(sptr, ":%s %s %s :*** Connect: Servidor %s %s %s.",
	...
	{
		sendto_one(sptr,
		    ":%s %s %s :*** Connect: Servidor %s no está configurado para linkar", me.name,
	...
	if (parc > 2 && !BadPtr(parv[2]))
	{
		...
		{
			sendto_one(sptr,
			    ":%s %s %s :*** Connect: Puerto ilegal", me.name,
	...
	{
		sendto_one(sptr, ":%s %s %s :*** Connect: falta puerto",
...
/* Evaluate deny link */
	for (deny = conf_deny_link; deny; deny = (ConfigItem_deny_link *) deny->next) {
		if (deny->flag.type == CRULE_ALL && !match(deny->mask, aconf->servername)
			...
			sendto_one(sptr,
				":%s %s %s :Connect: No permitido por configuración",
				me.name, IsWebTV(sptr) ? "PRIVMSG" : "NOTICE", parv[0]);
			return 0;
		}
	}
	...
	switch (retval = connect_server(aconf, sptr, NULL))
	{
	  ...
	  case -1:
		  ...
		  break;
	  case -2:
		  sendto_one(sptr, ":%s %s %s :*** Resolviendo hostname '%s'...",
		      me.name, IsWebTV(sptr) ? "PRIVMSG" : "NOTICE", parv[0], aconf->hostname);
		  break;
	  default:
		  sendto_one(sptr,
		      ":%s %s %s :*** Conexión a %s fallida: %s",
