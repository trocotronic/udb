...
ModuleHeader MOD_HEADER(m_sapart)
  = {
	"m_sapart",
	"$Id: m_sapart.c,v 1.1.4.10 2007-03-20 19:34:26 Trocotronic Exp $",
	"command /sapart", 
	"3.2-b8-1",
	NULL 
    };
		...
		*jbuf = 0;
		...
		if (comment)
		{
			sendto_one(acptr,
			    ...
			    commentx);
			sendto_realops("%s usa SAPART a %s en %s (%s)", sptr->name, parv[0],
				parv[1], comment);
		...
		else
		{
			sendto_one(acptr,
			    ...
			    IsWebTV(acptr) ? "PRIVMSG" : "NOTICE", acptr->name, parv[1]);
			sendto_realops("%s usa SAPART a %s en %s", sptr->name, parv[0],
				parv[1]);
