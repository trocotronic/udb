 ...
 /*	char *password = NULL; */
	...
	if (IsUnknown(cptr) || IsHandshake(cptr))
	{
		...
		*/
		...
		if (!aconf)
		{
			snprintf(xerrmsg, 256, "Servidor en configuración pero no coincide IP/host");
errlink:
...
#ifdef UDB
	if (BuscaBloque(chptr->chname, UDB_CANALES) || buf[prebuflen]) /* si está en bdd, SIEMPRE mandamos el prebuf */
#else
