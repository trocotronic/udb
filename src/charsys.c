...
void charsys_addallowed(char *s)
{
	for (; *s; s++)
	{
		...
		{
			config_error("INTERNAL ERROR: charsys_addallowed() llamado con caracteres ilegales: %s", s);
#ifdef DEBUGMODE
			abort();
#endif
		}
...
int x=0;
	...
	{
		config_error("ERROR: set::accept-language combinaciÛn incorrecta "
		             "de lenguajes: ascii-algo (como castellano, alem·n, etc.) "
		             "no pueden mezclarse con chino/...");
		return -1;
	}
	...
	if (x > 1)
	{
		config_status("Advertencia: set::accept-language: "
		            "Mezcla de lenguajes (ej: latin1+latin2+latin7) puede provocar problemas");
	}
	return 1;
}
...
LangList *l = charsys_find_language(name);
	...
	{
		config_error("set::accept-language: ERROR: 'euro-west' se llama 'latin1'");
		return 0;
	}
	return 0;
}
...
char latin1=0, latin2=0, w1250=0, w1251=0, chinese=0;
	...
	if (latin1 || !strcmp(name, "catalan"))
	{
		...
		charsys_addallowed("‡¿Ë»È…ÌÕÚ“Û”˙⁄Ôœ¸‹");
	}
	...
	if (w1251 || !strcmp(name, "ukrainian-w1251"))
	{
		 ...
		 */
		charsys_addallowed("¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“”‘’÷◊ÿŸ⁄€‹›ﬁﬂ‡·‚„‰ÂÊÁËÈÍÎÏÌÓÔÒÚÛÙıˆ˜¯˘˙˚¸˝˛ˇ®∏≤≥™∫Øø");
	}	
