...
static ConfigFile *config_parse(char *filename, char *confdata)
{
	...
	for(ptr=confdata;*ptr;ptr++)
	{
		switch(*ptr)
		{
			...
			case '{':
				if (!curce)
				{
					config_status("%s:%i: Sección sin nombre\n",
							filename, linenumber);
					continue;
				}
				else if (curce->ce_entries)
				{
					config_status("%s:%i: Ignorando sección extra\n",
							filename, linenumber);
					continue;
				}
			...
			case '\"':
				...
				if (curce)
				{
					if (curce->ce_vardata)
					{
						config_status("%s:%i: Ignorando información extra\n",
							filename, linenumber);
					}
					else
					{
			...
			default:
				...
				if (!*ptr)
				{
					if (curce)
						...
						config_error("%s: EOF antes de sección empezando en %i\n",
	...
	if (curce)
	{
		config_error("%s: EOF inesperado para variable de la línea %i\n",
			filename, curce->ce_varlinenum);
		config_entry_free(curce);
		config_free(curcf);
		return NULL;
	...
	{
		config_error("%s: EOF inesperado para sección de la línea %i\n",
				filename, cursection->ce_sectlinenum);
		config_free(curcf);
		return NULL;
	}
...
int	init_conf(char *rootconf, int rehash)
{
	...
	if (load_conf(rootconf) > 0)
	{
		if (config_test() < 0)
		{
			config_error("La configuración no ha pasado el test.");
#ifdef _WIN32
			if (!rehash)
				win_error();
#endif
	...
	else	
	{
		...
		config_error("No se pudo cargar la configuración del IRCd.");
		config_free(conf);
		conf = NULL;
		free_iConf(&tempiConf);
#ifdef _WIN32
...
int	load_conf(char *filename)
{
	...
	if (config_verbose > 0)
		config_status("Cargando el archivo de configuración %s ..", filename);
...
#ifndef _WIN32
		if (config_verbose > 1)
			config_status("Cargando los módulos en %s", filename);
	...
	{
		config_error("No se pudo cargar la configuración del archivo %s", filename);
		return -1;
	}	
}
...
char *pretty_time_val(long timeval)
{
	...
	if (timeval/86400)
		sprintf(buf, "%ld día%s ", timeval/86400, timeval/86400 != 1 ? "s" : "");
	if ((timeval/3600) % 24)
...
int	_conf_include(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!files.gl_pathc) {
		globfree(&files);
		config_status("%s:%i: include %s: archivo inválido",
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum,
			ce->ce_vardata);
		return -1;
	}	
...
int	_test_me(ConfigFile *conf, ConfigEntry *ce)
{
	...
	else
	{
		...
		if (cep->ce_vardata)
		{
			...
			{
				config_error("%s:%i: too long me::info, máximo. %i caracteres",
	...
	else
	{
		if (cep->ce_vardata)
		{
			...
			if ((l < 0) || (l > 254))
			{
				config_error("%s:%i: illegal me::numeric error (entre 0-254)",
					cep->ce_fileptr->cf_filename,
					cep->ce_varlinenum);
				errors++;
			}
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_error("%s:%i: línea en blanco",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum);
			errors++;
			continue;
		...
		if (!cep->ce_vardata)
		{
			config_error("%s:%i: me::%s sin parámetro",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum,
				cep->ce_varname);
			errors++;
...
int	_test_oper(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!cep->ce_entries)
		{
			...
			if (!cep->ce_vardata)
			{
				config_error("%s:%i: oper::%s sin parámetro",
					cep->ce_fileptr->cf_filename,
					cep->ce_varlinenum,
					cep->ce_varname);
				errors++; continue;
		...
		else
		{
			...
			if (!strcmp(cep->ce_varname, "from"))
			{
				for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next)
				{
					...
					if (!cepp->ce_vardata)
					{
						config_error("%s:%i: oper::from::%s sin parámetros",
			...
			{
				config_error("%s:%i: directriz desconocida oper::%s (sección)",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum,
						cep->ce_varname);
				errors++; continue;
			}
...
int	_test_class(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!cep->ce_vardata)
		{
			config_error("%s:%i: class item sin parámetro",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
			errors++; continue;
		}
	...
	if ((cep = config_find_entry(ce->ce_entries, "pingfreq")))
	{
		if (cep->ce_vardata)
		{
			...
			if (l < 1)
			{
				config_error("%s:%i: class::pingfreq con valor erróneo",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
	...
	if ((cep = config_find_entry(ce->ce_entries, "maxclients")))
	{
		if (cep->ce_vardata)
		{
			...
			if (!l)
			{
				config_error("%s:%i: class::maxclients con valor erróneo",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
	...
	if ((cep = config_find_entry(ce->ce_entries, "sendq")))
	{
		...
		{	
			...
			if (!l)
			{
				config_error("%s:%i: class::sendq con valor erróneo",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
	...
	if ((cep = config_find_entry(ce->ce_entries, "connfreq")))
	{
		if (cep->ce_vardata)
		{
			...
			if (l < 10)
			{
				config_error("%s:%i: class::connfreq con valor erróneo (<10)",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
	...
	if ((cep = config_find_entry(ce->ce_entries, "recvq")))
	{
		...
		{	
			...
			if (l < 512)
			{
				config_error("%s:%i: class::recvq con valor erróneo (<512)",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
...
int     _test_drpass(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!cep->ce_vardata)
		{
			config_error("%s:%i: falta parámetro en drpass:%s",
			 cep->ce_fileptr->cf_filename, cep->ce_varlinenum,
			 	cep->ce_varname);
			errors++; continue;
		}
...
int	_conf_ulines(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_status("%s:%i: uline item vacío",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum);
			continue;
		}
...
int	_test_ulines(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_error("%s:%i: uline item vacío",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum);
			errors++;
			continue;
...
int     _test_tld(ConfigFile *conf, ConfigEntry *ce)
{
        ...
        for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_error("%s:%i: tld item vacío",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum);
			errors++; continue;
		}
		...
		{
			config_error("%s:%i: falta parámetro en tld::%s",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum,
				cep->ce_varname);
			errors++; continue;
		}
		if (!strcmp(cep->ce_varname, "mask")) {
			...
			for (cep2 = cep->ce_entries; cep2; cep2 = cep2->ce_next)
			{
				...
				{
					config_error("%s:%i: opción desconocida tld::options::%s",
...
int	_test_listen(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!ip || !*ip)
	{
		config_error("%s:%i: listen: máscara errónea ip:port",
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
	if (strchr(ip, '*') && strcmp(ip, "*"))
	{
		config_error("%s:%i: listen: ip errónea, no puede ser '*')",
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
	if (!port || !*port)
	{
		config_error("%s:%i: listen: falta puerto en máscara",
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
#ifdef INET6
	...
	if (start == end)
	{
		...
		{
			config_error("%s:%i: listen: puerto erróneo (0-65535)",
				ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
			return 1;
		}
	}
	else 
	{
		if (end < start)
		{
			config_error("%s:%i: listen: rango de puertos erróneo (final menor que inicial)",
				ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
			return 1;
		}
		if ((start < 0) || (start > 65535) || (end < 0) || (end > 65535))
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!strcmp(cep->ce_varname, "options"))
		{
			if (!cep->ce_entries)
			{
				config_error("%s:%i: listen::%s sin parámetro",
					cep->ce_fileptr->cf_filename,
					cep->ce_varlinenum,
					cep->ce_varname);
				errors++;
...
int	_conf_allow(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!allow->class)
	{
		config_status("%s:%i: illegal allow::class, clase desconocida '%s'. Se usará la clase 'default'",
			cep->ce_fileptr->cf_filename,
			cep->ce_varlinenum,
			cep->ce_vardata);
...
int	_test_allow(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if ((cep = config_find_entry(ce->ce_entries, "maxperip")))
	{
		if (cep->ce_vardata)
		{
			...
			{
				config_error("%s:%i: allow::maxperip valor erróneo (>0)",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
	...
	if ((cep = config_find_entry(ce->ce_entries, "options")))
	{
		if (!cep->ce_entries)
		{
			config_error("%s:%i: allow::%s sin parámetro",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum,
				cep->ce_varname);
			errors++;
...
int     _test_except(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!strcmp(ce->ce_vardata, "ban")) {
		...
		{
			config_error("%s:%i: except ban sin máscara",
				ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
			return 1;
		}
		for (cep = ce->ce_entries; cep; cep = cep->ce_next)
		...
		if (!config_find_entry(ce->ce_entries, "mask"))
		{
			config_error("%s:%i: except throttle sin máscara",
				ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
			return 1;
		}
		for (cep = ce->ce_entries; cep; cep = cep->ce_next)
		...
		if (!used) {
			config_error("%s:%i: tipo desconocido unknown except %s",
				ce->ce_fileptr->cf_filename, ce->ce_varlinenum, 
				ce->ce_vardata);
			return 1;
		}
...
int	_test_vhost(ConfigFile *conf, ConfigEntry *ce)
{
	...
	else
	{
		...
		if ((at = strchr(vhost->ce_vardata, '@')))
		{
			...
			if (tmp != at)
			{
				config_error("%s:%i: vhost::vhost contiene una ident inválida",
					ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
				errors++;
			}
			host = at+1;
		...
		else
		{
			for (; *host; host++)
			{
				...
				{
					config_error("%s:%i: vhost::vhost contiene host erróneo",
						ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
					errors++;
					break;
				}
...
int _test_badword(ConfigFile *conf, ConfigEntry *ce) { 
	...
	if (!ce->ce_entries)
	{
		config_error("%s:%i: badword block vacío", 
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
	if (!ce->ce_vardata)
		...
		else 
		{
			...
			if (regex)
			{
				...
				if (errorcode > 0)
				{
					...
					config_error("%s:%i: badword::%s regex inválido: %s",
...
int _test_help(ConfigFile *conf, ConfigEntry *ce) { 
	...
	if (!ce->ce_entries)
	{
		config_error("%s:%i: help block vacío", 
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_error("%s:%i: help item vacío",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
			errors++; continue;
		}
	}
...
int _test_log(ConfigFile *conf, ConfigEntry *ce) { 
	...
	if (!ce->ce_entries)
	{
		config_error("%s:%i: log block vacío" , 
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_error("%s:%i: log item vacío",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
			errors++; continue;
		}
		if (!strcmp(cep->ce_varname, "flags")) {}
...
int	_conf_link(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!link->class)
	{
		config_status("%s:%i: illegal link::class, clase desconocida '%s'. Se usará la clase 'default'",
			cep->ce_fileptr->cf_filename,
			cep->ce_varlinenum,
			cep->ce_vardata);
...
int	_test_link(ConfigFile *conf, ConfigEntry *ce)
{
	...
	{
		config_error("%s:%i: link: nombre de servidor erróneo",
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
...
int     _test_ban(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (cep)
	{
		...
		{
			config_error("%s:%i: ban %s::action acción desconocida '%s'",
...
#define CheckNull(x) if ((!(x)->ce_vardata) || (!(*((x)->ce_vardata)))) { config_error("%s:%i: missing parameter", (x)->ce_fileptr->cf_filename, (x)->ce_varlinenum); errors++; continue; }
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!strcmp(cep->ce_varname, "kline-address")) {
			...
			{
				config_error("%s:%i: set::kline-address no puede ser la dirección de UnrealIRCd",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++; continue;
			}
			...
			{
				config_error("%s:%i: set::allow-userhost-change inválido",
					cep->ce_fileptr->cf_filename,
					cep->ce_varlinenum);
				errors++;
				continue;
			...
			else
			{
				...
				{
					if (!cepp->ce_varname)
						config_error("%s:%i: set::oper-only-stats item vacío",
							cepp->ce_fileptr->cf_filename,
							cepp->ce_varlinenum);
				}
			}
			...
			if (tempi < 1)
			{
				config_error("%s:%i: set::maxchannelsperuser erróneo (> 0)",
					cep->ce_fileptr->cf_filename,
					cep->ce_varlinenum);
				errors++;
				continue;
			...
			if (cep->ce_varname) {
				...
				for (p = cep->ce_vardata; *p; p++)
					if ((*p == '+') || (*p == '-'))
						warn = 1;
				if (warn) {
					config_status("%s:%i: advertencia: set::restrict-usermodes: sólo modos, no + o -.\n",
						cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				}
			}
		}
		else if (!strcmp(cep->ce_varname, "restrict-channelmodes"))
		{
			...
			if (cep->ce_varname) {
				...
				for (p = cep->ce_vardata; *p; p++)
					if ((*p == '+') || (*p == '-'))
						warn = 1;
				if (warn) {
					config_status("%s:%i: advertencia: set::restrict-channelmodes: sólo modos, no + o -.\n",
						cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				}
			}
		}
			...
			for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next) {
				...
				if (!strcmp(cepp->ce_varname, "timeout")) {
					...
					{
						config_error("%s:%i: set::dns::nameserver (%s) IP errónea",
							cepp->ce_fileptr->cf_filename, cepp->ce_varlinenum,
							cepp->ce_vardata);
						errors++;
						continue;
			...
			for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next) {
				...
				{
					config_error("%s:%i: opción desconocida set::anti-flood::%s",
						cepp->ce_fileptr->cf_filename,
						cepp->ce_varlinenum,
						cepp->ce_varname);
					errors++;
			...
			for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next) {
				...
				{
					config_error("%s:%i: opción desconocida set::options::%s",
						cepp->ce_fileptr->cf_filename,
						cepp->ce_varlinenum,
						cepp->ce_varname);
					errors++;
		...
		else if (!strcmp(cep->ce_varname, "cloak-keys"))
		{
			...
			if (i != 3)
			{
				config_error("%s:%i: set::cloak-keys: se necesitan 3 valores, no %i",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum,
					i);
				errors++;
				continue;
			...
			{
				config_error("%s:%i: set::cloak-keys: valores mayores que 10000",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
				continue;
			}		
			...
			{
				config_error("%s:%i: set::cloak-keys: valores menores que 2147483647 (2^31-1)",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
				continue;
			}
			...
			config_status("%s:%i: set::scan: advertencia: escáner de puertos deshabilitado, "
			...
			for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next)
			{
				...
				if (!strcmp(cepp->ce_varname, "connect-timeout") || !strcmp(cepp->ce_varname, "read-timeout"))
				{
					...
					config_error("%s:%i: directriz desconcida set::ident::%s",
...
int	_test_offchans(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!ce->ce_entries)
	{
		config_error("%s:%i: official-channels vacío", 
			ce->ce_fileptr->cf_filename, ce->ce_varlinenum);
		return 1;
	}
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		{
			config_error("%s:%i: official-channels: '%s' no es un nombre válido de canal.",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum, cep->ce_varname);
			errors++;
			continue;
		}
...
int _test_alias(ConfigFile *conf, ConfigEntry *ce) { 
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		if (!cep->ce_varname)
		{
			config_error("%s:%i: alias item vacío",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
			errors++; continue;
		}
		if (!cep->ce_vardata)
		{
			config_error("%s:%i: alias::%s sin parámetro",
				cep->ce_fileptr->cf_filename,
				cep->ce_varlinenum,
				cep->ce_varname);
			errors++; continue;
		...
		if (!strcmp(cep->ce_varname, "format")) {
			...
			errorcode = regcomp(&expr, cep->ce_vardata, REG_ICASE|REG_EXTENDED);
                        if (errorcode > 0)
                        {
                                ...
                                errorbuf = MyMalloc(errorbufsize);
                                regerror(errorcode, &expr, errorbuf, errorbufsize);
                                config_error("%s:%i: alias::format regex erróneo: %s",
 					cep->ce_fileptr->cf_filename,
 					cep->ce_varlinenum,
 					errorbuf);
                                errors++;
			...
			for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next) {
				if (!cepp->ce_vardata)
				{
					config_error("%s:%i: alias::format::%s sin parámetros",
						cepp->ce_fileptr->cf_filename,
						cepp->ce_varlinenum,
						cepp->ce_varname);
					errors++; continue;
...
int     _test_deny(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (!strcmp(ce->ce_vardata, "dcc"))
	{
		for (cep = ce->ce_entries; cep; cep = cep->ce_next)
		{
			if (!cep->ce_varname)
			{
				config_error("%s:%i: deny item vacío",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++; continue;
			}
			if (!cep->ce_vardata)
	...
	else if (!strcmp(ce->ce_vardata, "link"))
	{
		for (cep = ce->ce_entries; cep; cep = cep->ce_next)
		{
			...
			else if (!strcmp(cep->ce_varname, "rule"))
			{
				...
				{
					config_error("%s:%i: deny::%s expresión incorrecta: %s",
						cep->ce_fileptr->cf_filename,
						cep->ce_varlinenum,
