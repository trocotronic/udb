...
#ifdef UDB
void unrealdns_addtocache(char *name, void *binaryip, int length);
char *unrealdns_findcache_byaddr(struct IN_ADDR *addr);
#else
...
#ifdef UDB
DNSCache *unrealdns_findcache_byaddr_dns(struct IN_ADDR *addr)
{
unsigned int hashv;
DNSCache *c;
