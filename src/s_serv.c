...
CMD_FUNC(m_server)
{
 ...
 /*	char *password = NULL; */
	...
	if (parc < 4 || (!*parv[3]))
	{
		sendto_one(sptr, "ERROR :Faltan parámetros");
	...
	if (IsUnknown(cptr) || IsHandshake(cptr))
	{
		...
		strcpy(xerrmsg, "No hay configuración");
		...
		if (link->username && match(link->username, cptr->username)) {
			snprintf(xerrmsg, 256, "Username inválido '%s' a '%s'",
...
errlink:
			...
			sendto_one(cptr,
			    "ERROR :Link denegado (Sin configuración) %s",
			    inpath);
			...
			return exit_client(cptr, sptr, &me,
			    "Link denegado (Sin configuración)");
		}
nohostcheck:
		...
		{
			sendto_one(cptr,
			    "ERROR :Link denegado (Autentificación incorrecta) %s",
			    inpath);
			sendto_locfailops
			    ("Link denegado (Autentificación incorrecta) %s", inpath);
			return exit_client(cptr, sptr, &me,
			    "Link denegado (Autentificación incorrecta)");
		}
		...
		{
			sendto_realops
				("Link denegado %s, numeric inválido",
					get_client_name(cptr, TRUE));
			return exit_client(cptr, cptr, &me, "Numeric inválido");
		}
		...
		if (numeric)
		{
			...
			{
				sendto_locfailops("Link %s denegado, colisión de numerics",
					inpath);

				return exit_client(cptr, cptr, cptr,
				    "Colisión de numerics. Escoge otro");
			}
		}
		for (deny = conf_deny_link; deny; deny = (ConfigItem_deny_link *) deny->next) {
			if (deny->flag.type == CRULE_ALL && !match(deny->mask, servername)
				...
				sendto_ops("Conexión rechazada %s.",
...
CMD_FUNC(m_server_remote)
{
	...
	if ((bconf = Find_ban(servername, CONF_BAN_SERVER)))
	{
		...
		sendto_one(cptr, "ERROR :Servidor baneado (%s)", bconf->reason ? bconf->reason : "sin razón");
		return exit_client(cptr, cptr, &me, "Servidor baneado");
	}
	/* OK, let us check in the data now now */
	hop = TS2ts(parv[2]);
	...
	{
		sendto_realops
			("Link %s denegado, numeric inválido en %s",
				get_client_name(cptr, TRUE), servername);
		sendto_one(cptr, "ERROR :Numeric inválido (%s)",
			servername);
		return exit_client(cptr, cptr, &me, "Numeric remoto inválido");
	}
	strncpyzt(info, parv[parc - 1], REALLEN + 61);
	if (!cptr->serv->conf)
	{
	...
	{
		sendto_locfailops("Link %s denegado %s, configuración no permitida", cptr->name, servername);
		return exit_client(cptr, cptr, cptr, "Configuración de hub no permitida");
	}
	if (aconf->leafmask)
	{
		if (match(aconf->leafmask, servername))
		{
			sendto_locfailops("Link %s(%s) denegado por configuración de leaf", cptr->name, servername);
			return exit_client(cptr, cptr, cptr, "No permitido por configuración de leaf");
		}
	}
	...
	if (numeric)
	{
		...
		if (numeric_collides(numeric))
		{
			sendto_locfailops("Link %s(%s) denegado, colisión de numerics",
					cptr->name, servername);
...
int	m_server_synch(aClient *cptr, long numeric, ConfigItem_link *aconf)
{
	...
	if (aconf->options & CONNECT_ZIP)
	{
		if (cptr->proto & PROTO_ZIP)
		{
			...
			{
				zip_free(cptr);
				sendto_realops("Imposible de fijar configuración para link zip %s", get_client_name(cptr, TRUE));
...
CMD_FUNC(m_netinfo)
{
	...
	if (lmax > IRCstats.global_max)
	{
		...
		sendto_realops("Máximo usuarios en %li (por %s)",
		    lmax, cptr->name);
	}
...
void m_info_send(aClient *sptr)
{
	...
	sendto_one(sptr, ":%s %d %s :| Más información en \031\00312http://www.rallados.net", me.name, RPL_INFO, sptr->name);
#endif
	sendto_one(sptr,
	    ":%s %d %s :-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=", me.name,
	    RPL_INFO, sptr->name);
...
CMD_FUNC(m_connect)
{
	...
	{
		sendto_one(sptr,
		    ":%s %s %s :*** Conecta: Servidor %s no dispone de configuración", me.name,
...
CMD_FUNC(m_die)
{
	...
	sendto_ops("Servidor detenido a petición de %s", parv[0]);
